...
pub struct Cli {
    ...
    //
    // It sets all four `policy.defaults` to `allow` (`approval::UNCONFINED`) and
    // puts the turn's contract on `ExecMode::FullAccess` (`contract::unconfined`).
    //
    ...
    // file: it lasts the session and no longer, so it cannot be left on by accident
    // or committed into a repository. It is not requestable by the agent. And it is
    // marked on every frame it is in force, because the one thing worse than an
    // unconfined session is an unconfined session that looks like an ordinary one.
    //
    // `global` for the reason `-C` is: it is typed on either side of a subcommand.
    //
    // **The paragraphs above are `//` and not `///` on purpose.** A `///` here is
    // the flag's `--help` text, and on a `global` arg it prints on *every*
    // subcommand's help page — 0.40.0's `--profile` carried its release history
    // that way and had it taken out for the same reason. A help page describes the
    // binary in front of the reader; git and `CHANGELOG.md` hold the rest.
    /// Run unconfined: allow every act and take the sandbox out of the way, for
    /// this session only. Never written to a file.
