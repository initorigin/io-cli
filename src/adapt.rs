...
//! (`io-harness-0.81.0/src/plugin.rs:1097`), and the reason is the one this
...
/// (`io-harness-0.81.0/src/template.rs:279`), so what is translated is the
...
/// `templates` in every scope (`io-harness-0.81.0/src/plugin.rs:1097`), because
...
pub fn generate(bundle: &Path, name: &str, into: &Path) -> Result<Adapter, String> {
    ...
    // (`io-harness-0.81.0/src/plugin.rs:977`), and the refusal takes the whole
...
/// manifest in every scope (`io-harness-0.81.0/src/plugin.rs:977`), because a
...
/// (`io-harness-0.81.0/src/mcp.rs:301`), so a bundle declaring `sse` is declaring
