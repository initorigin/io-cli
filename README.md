...
| **A refusal that asks instead of ending the work** | When the agent is stopped by a *default* rather than by a rule you wrote, io asks — and your answer holds for the call, the session, or for good. A `[[policy.layers]]` deny you wrote still refuses, silently and always; so does a path outside the workspace root. It changes when io asks, never what io permits |
