...
| **A refusal that asks instead of ending the work** | When the agent is stopped by a *default* rather than by a rule you wrote, io asks — `y` for that call, `a` for the session, `w` to write the rule into your own configuration, in the user scope and never a workspace file. `/policy revoke` takes back what io wrote, and only what io wrote. A `[[policy.layers]]` deny you wrote still refuses, silently and always; so does a path outside the workspace root. It changes when io asks, never what io permits |
| **Unconfined when you mean it** | `io --full-access` sets every default to allow and takes the sandbox out of the way, which is the only thing that reaches a call the sandbox refuses structurally — a `bind()`, say. It is not in the `Shift+Tab` cycle, it is marked on every frame, it is never written to a file, and it still does not unlock `.env` |
