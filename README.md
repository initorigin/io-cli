...
| **A refusal that asks instead of ending the work** | When the agent is stopped by a *default* rather than by a rule you wrote, io asks — `y` for that call, `a` for the rest of the session. A `[[policy.layers]]` deny you wrote still refuses, silently and always; so does a path outside the workspace root. It changes when io asks, never what io permits |
