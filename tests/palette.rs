...
fn f1_equal_scores_keep_the_first_row_still_between_keystrokes() {
    // `c` matches TEN rows since 0.25.0: `copy`, `copy diff`, `contain`,
    // `clear`, `config`, `context`, `compact`, `cost` and now `commit` all begin
    // with it, and `mcp` merely holds one — which is the point of the arm, since a
    // scattered match ranks below every prefix. `attach`, which also merely held
    // one, stopped being a command in 0.13.1. `co` narrows to the eight that
